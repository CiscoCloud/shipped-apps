...
case class Config (
        ...
        val parser = new scopt.OptionParser[Config]("Shipped Analytics - Cassandra Demo") {
            ...
            opt[Int]('i',"interval") action { (x, c) => c.copy(interval = x)} text "interval in seconds for fetching load average data (for writer mode), 1 by default"
        };
        ...
        case Some(config) => {
...

            {
                ...
                logger.info("Starting with Cassandra node: " + config.node);
                val cluster = Cluster.builder().addContactPoint(config.node).build();
